...
function plateOutput(plate: Plate, props: IWorkerProps | null, stepIdx: number, forcedUpdate: boolean) {
  const result: any = {};
  result.id = plate.id;
...
export default function modelOutput(model: Model | null, props: IWorkerProps | null = null, forcedUpdate = false) {
  if (!model) {
